...
def _listar_arquivos_xlsx(caminho: str) -> list[str]:
    if os.path.isdir(caminho):
        return sorted(
            os.path.join(caminho, nome) for nome in os.listdir(caminho) if nome.lower().endswith(".xlsx")
        )
    return [caminho]
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Importa extrato(s) de conta corrente do Banco do Brasil (.xlsx) para o financiALL, de forma recorrente."
    )
    parser.add_argument("caminho", help="Arquivo .xlsx ou pasta com extratos")
    ...
    arquivos = _listar_arquivos_xlsx(args.caminho)
